...
export async function GET() {
    try {
        ...
        const response = await fetch(`http://${currentHost}:8000/api/pkm`);
        ...
        console.error("GET /api/pkm error:", error);
        return NextResponse.json(
            { message: "Failed to fetch PKM data", error: (error as Error).message },
            { status: 500 }
        );
    }
}